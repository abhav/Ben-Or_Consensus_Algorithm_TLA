...
\*Progress == \A a1 \in Procs: Cardinality({i \in Procs:(INPUT[i]=0)}) = N \/ Cardinality({i \in Procs:(INPUT[i]=1)}) = N => decided[a1] = -1 \/ decided[a1] = 2
...
\* Last modified Thu Nov 28 15:30:41 EST 2019 by abhavluthra
...
As i posted it piazza as well that if we need to run Progress with failures so we wrote 3 different property to check progress
-----------------------------------------------------------------------------
I Progress Property
-----------------------------------------------------------------------------
...
-----------------------------------------------------------------------------
Now if F=2, progress will not happen in the first round so progress is proved by negation that decided is always -1 or 2 and never 0,1
and whenever it reaches 0,1 the invariant is violated.
Progress == \A a1 \in Procs: Cardinality({i \in Procs:(INPUT[i]=0)}) = N \/ Cardinality({i \in Procs:(INPUT[i]=0)}) = N => decided[a1] = -1 \/ decided[a1] = 2

Then, We ran the model with N=4, F=1, MAXROUNDS = 3, INPUT=<<1,1,1,1>>, we got more than 2163 unique states and invariant get vioated ie decided=1.
Therefore progress happened
...
/\  decided = <<1, -1, -1, -1>>
/\  n_id = <<1, 2, 3, 4>>
/\  p1Msg = {[round |-> 1, val |-> 1, node_id |-> 1], [round |-> 1, val |-> 1, node_id |-> 2], [round |-> 1, val |-> 1, node_id |-> 3]}
/\  p1v = <<1, 1, 1, 1>>
/\  p2Msg = {[round |-> 1, val |-> 1, node_id |-> 1], [round |-> 1, val |-> 1, node_id |-> 2], [round |-> 1, val |-> 1, node_id |-> 3]}
/\  p2v = <<1, 1, 1, -1>>
/\  pc = <<"P2F", "CP2", "CP2", "P">>
...
-----------------------------------------------------------------------------
III Progress Property
-----------------------------------------------------------------------------
ProgressTermination == <>(\A a1 \in Procs: decided[a1] = 0 \/ decided[a1] = 1)   

It states that there will be termination only when decided became 0/1
